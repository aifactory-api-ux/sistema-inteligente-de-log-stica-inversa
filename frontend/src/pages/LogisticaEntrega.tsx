...
import { returnsApi, pickupApi, dropoffApi } from '../services/api';
import type {
  ReturnRequest,
  DecisionResult,
  TrackingTimeline,
...
const MOCK_TRACKING: TrackingTimeline = {
  ...
  events: [
    ...
    },
    {
      ...
      trace_id: 'TRACE-001',
      return_id: 'RET-2026-001',
      status: ReturnStatus.EN_TRANSITO,
...
export const LogisticaEntrega: React.FC = () => {
  const [searchParams] = useSearchParams();
  const navigate = useNavigate();
  const { currentReturn, fetchReturnById, setCurrentReturn } = useReturnsStore();
  ...
  const loadReturnData = async (id: string) => {
    ...
    try {
      ...
      setDropOffPoints(MOCK_DROP_OFF_POINTS);
    } catch (error) {
  ...
  const handleDownloadQR = () => {
    ...
    const qrData = `RET:${returnRequest.return_id}:${returnRequest.trace_id}`;
  ...
  const renderQRDropOff = () => {
    if (returnRequest?.return_method !== 'DROP_OFF') return null;
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="flex flex-col items-center p-6 bg-gray-50 rounded-xl border border-gray-200">
            ...
            <p className="text-xs text-gray-500 mt-4 font-mono">{returnRequest.return_id}</p>
            <PrimaryButtonCTA variant="outline" size="sm" onClick={handleDownloadQR} className="mt-4">
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="space-y-3">
                {dropOffPoints.map((point) => (
                  <div
                    key={point.point_id}
                    onClick={() => point.available && setSelectedDropOff(point)}
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-amber-800">Directrices de Embalaje</p>
                  <ul className="text-xs text-amber-700 mt-2 space-y-1">
                    <li>• Utiliza el embalaje original o uno similar</li>
                    <li>• Asegura que todos los artículos estén protegidos</li>
                    <li>• Incluye una copia de la etiqueta QR en el exterior</li>
                    <li>• Sella el paquete correctamente</li>
                  </ul>
                </div>
              </div>
            </div>
  ...
  const renderPickupScheduling = () => {
    if (returnRequest?.return_method !== 'PICKUP') return null;
                ...
                {pickupSlots
                  ...
                  .map((slot) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setPickupForm((prev) => ({ ...prev, preferred_time_slot: slot.time_slot }))
                      }
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {tracking.estimated_delivery && (
            <div className="text-sm text-gray-500">
              ...
              Entrega estimada: {formatDate(tracking.estimated_delivery)}
            </div>
          )}
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              {returnRequest?.return_method === 'DROP_OFF' && renderQRDropOff()}
              {returnRequest?.return_method === 'PICKUP' && renderPickupScheduling()}
...
export default LogisticaEntrega;